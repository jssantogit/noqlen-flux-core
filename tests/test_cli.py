...
def test_staging_execute_fake_approved_dry_run_works(tmp_path, capsys) -> None:
    ...
    assert main(["staging", "apply", "fake", "approved", "--workspace", str(workspace), "--dry-run"]) == 0
...
def test_staging_execute_fake_approved_apply_works(tmp_path, capsys) -> None:
    ...
    assert main(["staging", "apply", "fake", "approved", "--workspace", str(workspace), "--apply"]) == 0
...
def test_staging_execute_fake_quarantine_dry_run_works(tmp_path, capsys) -> None:
    ...
    assert main(["staging", "apply", "fake", "quarantine", "--workspace", str(workspace), "--dry-run"]) == 0
...
def test_staging_execute_fake_rejected_dry_run_works(tmp_path, capsys) -> None:
    ...
    assert main(["staging", "apply", "fake", "rejected", "--workspace", str(workspace), "--dry-run"]) == 0
...
def test_staging_execute_fake_delete_eligible_dry_run_works(tmp_path, capsys) -> None:
    ...
    assert main(["staging", "apply", "fake", "delete-eligible", "--workspace", str(workspace), "--dry-run"]) == 0
...
# --- Slskd transfer plan CLI tests ---


def test_transfer_plan_slskd_track_offline_returns_error(capsys) -> None:
    assert main(["transfer", "plan", "slskd", "track", "--artist", "Example Artist", "--title", "Example Track", "--offline"]) == 1
...
def test_transfer_plan_slskd_album_offline_returns_error(capsys) -> None:
    assert main(["transfer", "plan", "slskd", "album", "--artist", "Example Artist", "--album", "Example Album", "--offline"]) == 1
...
def test_transfer_plan_slskd_track_default_is_offline(capsys) -> None:
    assert main(["transfer", "plan", "slskd", "track", "--artist", "Example Artist", "--title", "Example Track"]) == 1
...
def test_transfer_plan_slskd_album_default_is_offline(capsys) -> None:
    assert main(["transfer", "plan", "slskd", "album", "--artist", "Example Artist", "--album", "Example Album"]) == 1
...
def test_transfer_plan_slskd_track_allow_network_no_url_returns_error(capsys) -> None:
    assert main(["transfer", "plan", "slskd", "track", "--artist", "Example Artist", "--title", "Example Track", "--allow-network"]) == 1
...
def test_transfer_plan_slskd_track_api_key_not_printed(capsys, monkeypatch) -> None:
    monkeypatch.setenv("TEST_SLSKD_KEY", "super-secret-key-12345")

    assert main([
        "transfer", "plan", "slskd", "track",
        "--artist", "Example Artist",
        "--title", "Example Track",
        "--allow-network",
        "--url", "http://localhost:5000",
...
def test_transfer_plan_slskd_album_api_key_not_printed(capsys, monkeypatch) -> None:
    monkeypatch.setenv("TEST_SLSKD_KEY", "super-secret-key-12345")

    assert main([
        "transfer", "plan", "slskd", "album",
        "--artist", "Example Artist",
        "--album", "Example Album",
        "--allow-network",
        "--url", "http://localhost:5000",
...
def test_transfer_plan_slskd_track_mocked_network_succeeds(capsys, monkeypatch) -> None:
    import json
...
def test_transfer_plan_slskd_album_mocked_network_succeeds(capsys, monkeypatch) -> None:
    import json
    ...
    assert "search: success" in output
    assert "download-planning:" in output
    assert "transfer-planning:" in output
    assert "album-user" in output
    assert "01 Intro.flac" in output
    assert "02 Track.flac" in output


def test_transfer_plan_slskd_track_mocked_with_score(capsys, monkeypatch) -> None:
    import json
...
def test_transfer_plan_slskd_track_mocked_score_min_blocks(capsys, monkeypatch) -> None:
    import json
...
def test_transfer_plan_slskd_track_mocked_max_files_blocks(capsys, monkeypatch) -> None:
    import json
...
def test_transfer_plan_slskd_track_mocked_max_total_bytes_blocks(capsys, monkeypatch) -> None:
    import json
...
def test_transfer_plan_slskd_track_mocked_locked_files_blocked(capsys, monkeypatch) -> None:
    import json
...
def test_transfer_plan_slskd_track_mocked_allow_locked_succeeds(capsys, monkeypatch) -> None:
    import json
...
def test_transfer_plan_slskd_track_mocked_candidate_index_out_of_range(capsys, monkeypatch) -> None:
    import json
    ...
    output = capsys.readouterr().out
    assert "transfer-planning: failed" in output
    assert "out of range" in output.lower()


def test_transfer_plan_slskd_track_mocked_allowed_extension_blocks(capsys, monkeypatch) -> None:
    import json
...
def test_transfer_plan_slskd_track_mocked_priority_high(capsys, monkeypatch) -> None:
    import json
