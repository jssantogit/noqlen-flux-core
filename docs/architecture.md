...
The CLI also supports slskd search-to-transfer planning: `transfer plan slskd track` and `transfer plan slskd album`. These commands build `SearchQuery` objects, instantiate `SlskdProvider` directly (as CLI adapter), call `SearchService`, optionally score candidates, select a candidate by `--candidate-index` or default to the first, build a `DownloadRequest` with constraints, call `DownloadPlanningService` to get a `DownloadPlan`, then call `TransferPlanningService` to get a `QueuePlan`, and render all three results. Network access is disabled by default; `--allow-network` is required for real search. Planning options include `--score`, `--candidate-index`, `--score-min`, `--max-files`, `--max-total-bytes`, `--allow-locked`, `--allowed-extension`, and `--priority`. Core services remain provider-agnostic; `SearchService`, `DownloadPlanningService`, and `TransferPlanningService` depend only on generic contracts and Flux models. A future `NativeSoulseekProvider` can implement the same flow without core changes. UI/search surfaces can use `SearchCandidate` + `DownloadPlan` + `QueuePlan` for candidate selection and action preview. QueuePlan is a preview, not execution.
